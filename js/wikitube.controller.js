...
import { getRandomInt, getRandomIntInclusive } from './services/util.service.js'
...
function searchAndRender(txt) {
	//Youtube
	wtService.getVideos(txt).then(videos => {
	...
	//Wiki
	wtService.getWiki(txt).then(renderWikis)
}

function renderVideoList(videos) {
	const strHTMLs = videos.map(({ id, title, thumbnail }) => {
		return `
            <article onclick="app.onChangeVideo('${id}')" class="video-card">
...
function onOpenThemeModal() {
	askUser().then(color => (document.querySelector('body').style.backgroundColor = color))
}
